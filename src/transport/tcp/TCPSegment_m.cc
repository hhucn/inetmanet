//
// Generated file, do not edit! Created by opp_msgc 4.4 from transport/tcp/TCPSegment.msg.
//
...
Sack::Sack(const Sack& other) : cObject(other)
{
    copy(other);
}
...
void Sack::parsimPack(cCommBuffer *b)
{
    doPacking(b,this->start_var);
    doPacking(b,this->end_var);
}

void Sack::parsimUnpack(cCommBuffer *b)
{
    doUnpacking(b,this->start_var);
    doUnpacking(b,this->end_var);
}

unsigned int Sack::getStart() const
{
    return start_var;
}

void Sack::setStart(unsigned int start)
{
    this->start_var = start;
}

unsigned int Sack::getEnd() const
{
    return end_var;
}

void Sack::setEnd(unsigned int end)
{
    this->end_var = end;
}
...
{
    return dynamic_cast<Sack *>(obj)!=NULL;
}
