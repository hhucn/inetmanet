...
MulticastRoutes RoutingTable::broadcastRoutesFor(const IPAddress& dest) const
{
	Enter_Method("broadcastRoutesFor(%s)=?", dest.str().c_str());
	...
	{
		MulticastRoute r;
		r.interf = ift->getInterface(i);
		if (!r.interf->isBroadcast()) continue;
		r.gateway = IPAddress::ALLONES_ADDRESS;
		res.push_back(r);
	}
	return res;
}
